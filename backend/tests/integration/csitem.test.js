...
describe("/courseSubItems", () => {
  ...
  describe("GET /", () => {
    it("should return all CourseSubItems", async () => {
      ...
      expect(res.body.data.courseSubItems).toHaveProperty("subItemType", dbCourseSubItems.subItemType);
      expect(res.body.data.courseSubItems.some(csi => csi.subItemTitle === "Introduction%20to%20Programming")).toBeTruthy();
      expect(res.body.data.courseSubItems.some(csi => csi.subItemTitle === "Introduction%20to%20Programming%20-%201")).toBeTruthy();  
    });
  });
  ...
  describe("GET /:id", () => {
    it("should return subItem if valid id is passed", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.data.courseSubItem).toHaveProperty("subItemTitle", prblm.subItemTitle);
    });
  ...
  describe("POST /", () => {
    ...
    it('should return the courseSubItems if it is valid', async() => { 
      ...
      expect(res.body.data.courseSubItem).toHaveProperty("subItemTitle","Introduction%20to%20Programming");      
    });
  });
});
